...
### 2G'' 升级规则增强（run 2026-05-14-1426-cp-upgrade-buttons / M2 公开 API 出口）

> 红蓝二轮决议：GO_with_5_constraints。本组覆盖 M2（公开 API `/upgrade/check` response 出口接线）。依赖 M1 全部交付。事实层修正：**当前 `/upgrade/check` 没有 response 级 Redis 缓存**，只有 `app_registry` 进程内 dict 缓存 `CpApp` 对象（不含规则）；M2 不新增 response 缓存，只做 schema 兼容性 smoke。

- [✓] **2.55** `UpgradeCheckResponse` schema 加 `popup_buttons` + `PopupButtonResolved`；专属测试 9 用例（任务 ID **M2.T1**）
- [✓] **2.56** `routers/app.py::upgrade_check` 接线 i18n_fallback helper（删 inline 死表）（任务 ID **M2.T2**）
- [✓] **2.57** `routers/app.py::upgrade_check` 输出 popup_buttons via resolve_popup_buttons；测试覆盖含 i18n + 丢弃 + 顺序（任务 ID **M2.T3**）
- [✓] **2.58** Play 渠道防漏出 e2e 3 用例（任务 ID **M2.T4**）
- [✓] **2.59** 缓存兼容性 smoke 3 用例（本期不引入 response 缓存）（任务 ID **M2.T5**）
- [✓] **2.60** docs/api.md 已刷（无 diff，M2.T1 已同步）+ 全量 pytest 164 绿 + import-linter ok（任务 ID **M2.T6**）
